...
#ifndef COLDSPACE_C_SOUNDEMITTER_H
#define COLDSPACE_C_SOUNDEMITTER_H
#include "C_Base.h"
        ...
        for (int i = 0; i < Max_EntitySounds; ++i){
            ...
            while (frameNum < SoundParameters::Max_SoundFrames){
                ...
                if (pos != std::string::npos){
                    frame = StringToInt(frames.substr(0, pos));
                    frames.erase(0, pos + frame_delimiter.length());
                } else {
                    frame = StringToInt(frames);
...
#endif //COLDSPACE_C_SOUNDEMITTER_H
