...
Entity_Manager::Entity_Manager(SystemManager* l_sysMgr,
...
//creates default-components entity
int Entity_Manager::AddEntity(const Bitmask& l_mask){
...
//Factory-entity-function, loading blueprints.
int Entity_Manager::AddEntity(const std::string& l_entityFile){
    int EntityId = -1;
...
bool Entity_Manager::RemoveEntity(const EntityId& l_id){
    ...
    //notifies entity removal
    m_systems->RemoveEntity(l_id);
    return true;
}

bool Entity_Manager::AddComponent(const EntityId& l_entity, const Component& l_component)
{
...
bool Entity_Manager::RemoveComponent(const EntityId& l_entity, const Component& l_component)
{
    auto itr = m_entities.find(l_entity);
    if (itr == m_entities.end()){ return false; }
    // Found the entity.
...
bool Entity_Manager::HasComponent(const EntityId& l_entity, const Component& l_component)
{
    auto itr = m_entities.find(l_entity);
    if (itr == m_entities.end()){ return false; }
    return itr->second.first.GetBit((unsigned int)l_component);
}
//purges all entities, removing all components and containers.(in this order)
