...
//sets default members, sends map to shared context gameMap, loads tiles from tiles.cfg
...
//purge configurations and tilesets, reset shared context gameMap.
Map::~Map(){
    PurgeMap();
    PurgeTileSet();
...
//gets a specific tile using 1D coordinates.
...
//if loadnextmap is true, then purges map, and switches to the next one, otherwise, send GameOver State.
